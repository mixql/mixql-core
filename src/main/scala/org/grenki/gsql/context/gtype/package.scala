...
package object gtype {
  ...
  /** implicit conversions from scala types to gtypes
    * or gtypes to scala types
    */
  ...
  /** conversions any gtype to gtype you need
    * (if possible)
    */
      ...
      a match {
        ...
        case string(value, _) => 
          if (value.toLowerCase == "true") 
            ...
            throw new ClassCastException(s"cannot convert string \"$value\" to bool")
        case Null => 
          throw new NullPointerException("cannot convert null to bool")
        case value => 
          throw new ClassCastException(s"cannot convert ${value.getClass.getSimpleName} to bool")
      }
    }
